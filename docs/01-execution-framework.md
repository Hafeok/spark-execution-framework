# Kiln: the Execution Framework

> Kiln, an execution-pillar framework implementation. Conformant to the Two Pillars **Execution Contract**; built to satisfy the **Product Framework**'s conformance ladder (Described → Realised → Verified → Delivered) at the execution end. It is to execution what the Product Framework is to specification: one opinionated, conformant way to satisfy the foundation. Substrate-neutral in its obligations; the DGX Spark is the reference substrate.
...
2. **The Work-Unit Interface** — the seam. Two per-run schemas (WorkUnit out, VerdictEvent in) and two emit-points, plus one out-of-band **CapabilityManifest** the executor publishes so producers can match a unit's requirements before dispatch; no shared runtime surface. Verdict-as-event: the executor emits and is done, knowing no consumer.
...
| **Verification** (declared verifier, declared verdict) | Who validates each output, with what authority, producing a verdict in a declared vocabulary; no output accepted without one. | The executor walks the sealed cell-DAG; each cell-gate is a verification against a **protected oracle the worker cannot write** (ADR-076). Cell-verdicts reduce to a unit-verdict in the declared vocabulary — *accepted / rejected / escalate* — never a boolean in an exit code. Done is computed, not claimed (ADR-071). **Admission is a distinct gate that precedes verification:** a unit whose derived requirements the box cannot cover reaches the boundary and is answered `not-admitted` with the concrete `missing-capabilities` — it *never ran*, so `tier-ran` and `cell-results` are absent. Admission failure is not execution failure; the two are different verdicts because a higher tier fixes the latter and can never fix the former. |

### Group 2 — the grants (what a worker is permitted, and where)

| Block | Completeness requirement | Discharged by |
| --- | --- | --- |
| **Capabilities** (read / write / call / spawn floor) | Every action category a worker may take declared explicitly; held exactly to task need; nothing implicit. | Each Worker-role declares its capability set over the floor. A coder cell-worker holds **read** (its frozen bundle), **write** (its declared repository working tree — a per-run worktree or clone of the WorkUnit's `artifact-delivery.repository`), and **call** (its declared effect tools, which in `pull-request` delivery *includes the forge API call that opens the PR*); it does **not** hold **spawn**. The executor itself holds **spawn** (it dispatches cell-workers) but not **write** to source. Excess capability is a decomposition signal (Rule 1). These same capabilities are what the executor **publishes** in its CapabilityManifest and **admits against** at the boundary: a unit whose derived requirements exceed them yields `not-admitted` with the `missing-capabilities` distance (see *Publishing capabilities* below) — declared, matchable, never implicit. |
| **Environment** (boundary by construction, not instruction) | The execution boundary declared per worker: reachable filesystem, permitted network, host/peer reachability, what is destroyed at unit end — enforced structurally. | **Per-work-unit ephemeral isolation.** Each admitted work-unit runs in its own sandbox: its writable working tree is a **per-run worktree or clone of the declared repository** (`artifact-delivery: repository`) — or a private scratch workspace under `inline` delivery — the frozen bundle mounted read-only, **no network except declared destinations** (the repository host and, for `pull-request` delivery, the forge), no reach to the host or to other in-flight units, and the whole sandbox **destroyed when the unit reaches a verdict**. Isolating concurrent runs against one repository is thus the sandbox's job, invisible to the seam. The sealed cell-DAG lives entirely inside one such boundary — this is *why* the cell interior is sealed: it is one isolation domain. On a single shared box running many units batched against one resident model, this per-unit sandbox is the structural separation the contract demands; the model is shared, but each unit's effects, working tree, and credentials are not. |
...
| **Credentials** (work-unit-scoped, short-lived, never standing) | Access to external systems scoped to the work-unit, least-privilege, time-bounded, revoked at unit end; the worker holds a reference, not a standing secret. | The WorkUnit carries a **credential-grant reference**, not a secret — and its `artifact-delivery.repository` block deliberately carries **no token**, because the unit is frozen, content-hashed, stored, and re-emitted, so a secret inline would be baked into an immutable, logged artifact. At admission the isolation boundary exchanges the reference for short-lived, least-privilege credentials scoped to the unit's declared effect destinations only — including **repository push access and, for `pull-request` delivery, forge-API access** — and **revokes them when the sandbox is destroyed at verdict**. The executor never injects a standing credential; authority derives from the unit being executed, not from the executor's identity. A unit that needs no effect destination gets no credential. |
...
| **Output Contract** (declared shape + destination) | Each worker's output declared, with where it lands and how it reaches the next stage; state crosses explicitly, no hidden channel. | The VerdictEvent schema (shape) emitted to the event stream (destination), plus the unit's produced work landing per `artifact-delivery`: `inline` returns artifact bodies by value in the event; `repository` lands the run in the **declared git repository** per its integration method (`push-branch` or `pull-request`) and the event carries `delivery-result` (branch, commit, `pr-url`) — refs, not payloads, with the commit SHA as the content hash over the produced tree. State crosses only via these declared channels; the per-unit sandbox guarantees there is no shared mutable store through which two units could exchange state off-contract, and Kiln writes to no repository the unit did not declare. |
| **Transition Contract** (verdict → consequence; the autonomy level) | Every verdict bound to a consequence — advance / halt / retry / escalate — with the human-free bindings declared. The autonomy level *is* this contract's shape. | `next-consequence` on the VerdictEvent: advance / halt / retry / escalate. Escalation re-enqueues the whole unit one tier up (deferral across the developer switch when the next tier is night-only). A **`not-admitted`** verdict binds to `halt` or `escalate`, never `advance` — the consumer re-routes to a box that covers the distance or surfaces to a human; Kiln does not advance the ladder against itself, since a higher tier cannot add a missing capability. The human-free set lives in `acceptance-class`: `auto-commit-if-green` is a verdict→advance binding that proceeds without a human (Level 5 for that unit kind); `needs-verdict` binds *accepted* to escalate-to-human (Level 4). The autonomy level is thus per-unit-kind, declared, not a global setting. |
...
Running the contract's own end-to-end test on this framework: a **cell-worker** (Worker), holding declared **read/write/call capabilities** (Capabilities), is dispatched into a **per-unit ephemeral sandbox** (Environment), able to invoke declared **effect tools** (Tools) using **unit-scoped credentials exchanged from a grant-reference** (Credentials). It receives the **frozen by-value bundle** (Input Contract), does its work, and writes to its **declared repository working tree** while the executor emits a **VerdictEvent** (Output Contract). Each cell-gate is a **verification against a protected oracle** (Verification), reduced to a unit-verdict. **`next-consequence`** binds that verdict to advance / halt / retry / escalate (Transition Contract). Every reference resolves; every grant traces to a task need; every output is verified; every verdict has a consequence. **The system is closed — all eight blocks discharged.**

---

## Publishing capabilities: the CapabilityManifest

The **Capabilities** block requires every action category to be *explicit and declared*. Kiln makes those declarations **published and matchable** by emitting a **CapabilityManifest** — the one seam artifact the execution side authors (the complement of producer-owns, not a breach of it). This lets a producer discover a unit's executability at *planning time* rather than as a runtime surprise, and makes a Kiln instance selectable as a production deploy target: a target is a box whose manifest covers the unit's requirements.

**What the manifest declares (normative for matching).** The `bindings` the box can serve — on the Spark, exactly the resident/validated bindings in [`bindings/`](../bindings/), pinned with the same identity fields a WorkUnit binding carries; the `delivery` modes and, for `repository`, the `url-schemes` (`file`, `https`, `ssh`), `integration-methods` (`push-branch`, `pull-request`), and reachable `forges`; the `shape-languages` it can validate against; and the `gate-kinds` it can run. `operational` hints (queue-depth, current QUEUE/EXPLORER mode) are **advisory only** — they may order routing among boxes that all match, but MUST NOT make a unit inexecutable. Capacity is a queueing concern; a stale queue reading must never make executability flicker.

**Requirements, distance, reach.** A unit's *requirements* are derived from the unit itself — its binding, delivery mode + URL scheme + integration method, cell shape-languages, cell gate-kinds — needing no extra declaration. The *distance* is `requirements(unit) − capabilities(box)`; **empty distance = executable**. A unit's *reach* is the set of boxes whose manifests cover it; a minimal inline-delivery unit reaches everywhere, a unit demanding an exotic quantization plus GitHub-PR delivery reaches few. Reach is a property of how the unit was specified, dovetailing with the funnel: demanding less widens reach.

**The manifest is pre-flight; admission stays authoritative.** A published manifest is a snapshot; the box's state at dispatch may differ. A successful match therefore **does not guarantee** admission — the boundary remains the authority, and a `not-admitted` verdict after a match is expected and correct. The manifest exists to make inexecutability *predictable*, not impossible. When a `not-admitted`'s missing capability appears in the manifest, the manifest was stale — a signal to refresh it, not to advance a ladder. This is why the CapabilityManifest crosses **out of band**, not per run: it is a self-description that changes on the box's own timescale, decoupled from any single unit's dispatch.
...
- **[The Work-Unit Interface](../interface/work-unit-interface.md)** — the seam: WorkUnit (by value, hash-identified) out, VerdictEvent (self-describing, fire-and-forget) in, CapabilityManifest (published, matched before dispatch) out of band; the `dispatch`/executor cut.
- **[ai-development-contracts](https://github.com/Hafeok/ai-development-contracts)** — the shared wire schemas this framework consumes and (for the CapabilityManifest) authors; conformance declared in [`conformance/contracts-conformance.md`](../conformance/contracts-conformance.md).
