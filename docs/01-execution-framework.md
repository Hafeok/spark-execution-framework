...
| **Output Contract** (declared shape + destination) | Each worker's output declared, with where it lands and how it reaches the next stage; state crosses explicitly, no hidden channel. | The VerdictEvent schema (shape) emitted to the event stream (destination), plus the unit's effect-tool writes to its **declared output workspace**. State crosses only via these declared channels; the per-unit sandbox guarantees there is no shared mutable store through which two units could exchange state off-contract. |
