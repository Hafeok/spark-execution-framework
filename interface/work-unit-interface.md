...
> The contract between a specification-pillar implementation (product-cli) and an execution-pillar implementation (Kiln; reference substrate DGX Spark). Two data schemas and two emit-points. No shared runtime surface. Either side may be rebuilt from scratch against this contract alone.
...
product-cli  ──emits──▶  WorkUnit            (by value · bundle inlined · hash = identity)
                              │
                              ▼
                  [ Kiln executor: admit → walk sealed cell-DAG → unit-verdict ]
                              │
